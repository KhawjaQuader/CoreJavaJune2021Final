...
5) Sign in to github, create a repository, keep in public, submit. A remote https:// will be created.
Follow the steps from 7 below.
...
6) Sign in to github, create a repository, keep in public, submit. A remote https:// will be created.
Follow the steps from 7 below.
...
7) Configure User Name: git config --global user.name “yellowBird”
8) Configure User Email: git config --global user.email “yellowBird”
9) Check User Name: git config user.name
10) Check User Email: git config user.email
11) To see both: git config --list


Necessary command (One time only)
...
15) (don't work for command prompt) You don’t need to write the complete command. Type ec, then press tab button, then type -wo, then press tab button.
16) List (to see the content of eclipse-workspace): ls, for command prompt: dir
17) Choose which project you want to publish in Github. (don't work for command prompt) Type Ho, then press tab button: cd HomeWorkCoreJavaJune2021
...
27: To push the local repository to the remote origin: git push origin master
